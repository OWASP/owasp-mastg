...
| Malware danger | Malware samples can infect a device, but not when you can flash the firmware and clear out the storage, this should not be a problem. Be aware that there are malware samples that try to exploit the USB bridge | Malware samples can infect an emulator, which can be undone by removing the emulator instance and its storage. Be waware that there are malware Proof of Concepts which try to attack the hypervisor. |
