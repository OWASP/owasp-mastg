...
- Setting the [android:usesCleartextTraffic](https://developer.android.com/guide/topics/manifest/application-element#usesCleartextTraffic) attribute of the \<application\> tag in the AndroidManifest.xml file. Note that this flag is ignored in case a network security config is configured.
...
- Using low level libraries (e.g. [Socket](https://developer.android.com/reference/java/net/Socket) to set up a custom HTTP connection.
