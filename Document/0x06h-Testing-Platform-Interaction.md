...
There is a caveat to above example. In recent versions of iOS, new APIs have been introduced for accessing Photos, which do not require users' permission. For example,  [`UIImagePickerController`](https://developer.apple.com/documentation/uikit/uiimagepickercontroller "UIImagePickerController") (iOS 11+) and its modern [replacement](https://developer.apple.com/videos/play/wwdc2020/10652/ "replacement") [`PHPickerViewController`](https://developer.apple.com/documentation/photokit/phpickerviewcontroller "PHPickerViewController") (iOS 14+). These APIs runs as a separate process from your app and on using them, the app cannot see the user's whole Photo library, it gets a read-only access just for the images selected by the user.
