...
While "traditional" server-side sessions are sometimes used in mobile apps, stateless token-based approaches are becoming increasingly popular for a variety of reasons:

- They eliminate the need to store session state on the server, improving scalability and performance.
- Using Tokens decouples authentication from the app. Tokens can be generated by and authentication server and the authentication scheme can be changes seamlessly.
