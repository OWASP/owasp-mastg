...
The above steps place the following output in pasteboard to be pasted in a `.js` file and fed into `frida -U -l`.
...
The above code generated by `jadx-gui` hooks the `a` method within the `sg.vantagepoint.a.a` class and logs it's entry parameters and return values.
