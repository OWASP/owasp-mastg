...
- MAC addresses (TODO IMPLEMENT IN SWIFT, OBJ-C)
- the UDID (TODO IMPLEMENT IN SWIFT, OBJ-C)
- Any keychain binding which are unprotected by (TODO DEFINE THE WAYS TO COUPLE IT!)
- Any filesystem based binding which are unprotected by (TODO DEFINE THE WAYS TO COUPLE IT!)
...
- As simulators use UUIDs to identify themselves, you could make it easer to locate the storage by creating a debug point and on that point execute `po NSHomeDirectory()`, which will reveal the location of where the simulator stores its contents.
...
- Use a token based device authentication (InstanceID from Google) to reassure that the same instance of the app is used.
