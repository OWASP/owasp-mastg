...
iOS 8 introduced changes to the way an application is stored on a device. In the earlier versions of iOS applications would be unpacked to a folder in the /var/mobile/applications/ folder. An application would then be identified by its UUID (Universal Unique Identifier), a 128-bit number that will be the name of the folder where the application can be found. Since iOS 8 release static bundle and application data folders are now stored in different locations in the file system. These folders contain information that we must be closely examined during application security assessments.
...
- `/var/mobile/Containers/Data/Application/[UUID]/Documents` contains all the data stored for the application itself. The creation of this data is initiated by the application end user.
- `/var/mobile/Containers/Data/Application/[UUID]/Library` contains the files used by the application e.g. caches, preferences, cookies, property list (plist) configuration files, etc.
- `/var/mobile/Containers/Data/Application/[UUID]/Temp` contains temporary files which are not needed between application launches.
