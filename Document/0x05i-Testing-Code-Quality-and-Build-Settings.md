...
If the application is a high risk application, you will end up vetting the library manually. In that case, there are a specific requirements for native code, which you can find in the chapter ["Testing Code Quality"](0x04h-Testing-Code-Quality.md). Next to that, it is good to vet whether all best practices for software engineering are applied.
