...
def get_testcases_info(chapter_file_name, chapter):
    ...
    for testcase in testcases:
        ...
        testcases_info.append(TestCase(mstg_id, link, title, platform, mstg_ids, tools, overview))
...
def get_links_to_other_chapters(chapter):
    links = chapter.find_all(['a'], {"href": re.compile("0x0.*\.md#.*")})
    return [link.get('href') for link in links]

def get_all_links_to_tools(chapter):
    links = chapter.find_all(['a'], {"href": re.compile("0x08.*\.md#.*")})
    return [link.get('href') for link in links]
