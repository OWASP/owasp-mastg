...
- Other custom C/C++ libraries might not have stack canaries enabled because they lack the necessary compiler flags (`-fstack-protector-strong`, or `-fstack-protector-all`) or the libraries were optimized out by the compiler. See the [Evaluation](#evaluation) section for more details.
...
When evaluating this please note that there are potential **expected false positives** and the test case should be considered as passed. But this would require a manual review of the original source code and the compilation flags used. The following examples cover some of the false positive cases that might be encountered:
