...
class BannerPlugin(BasePlugin):
    def on_config(self, config):
        self.issue_mapping = github_api.get_issues_for_test_refactors()
        print(self.issue_mapping)
        self.xx = self.get_v1_tests_data()
        self.mastg_v1_tests_metadata, self.mastg_v1_mapping = self.get_v1_tests_data()


    def get_v1_tests_data(self):
    ...
    refactor_banner = """
    ??? warning "This test needs updating"
        This test has not been refactored yet to comply with the new <a href="https://docs.google.com/document/d/1veyzE4cVTSnIsKB1DOPUSMhjXow_MtJOtgHeo5HVoho/edit?tab=t.0#heading=h.ue8tn3i2ff0">OWASP MASTG v2 structure</a>. Please help us out!
        
        Check the following items for more info: 
    ...
    def get_tests_refactor_banner(self, meta):
        links = self.issue_mapping.get(meta.get("id"))
        if links:
            alllinks = []
            for link in links:
                alllinks.append("<a href='{}'>{}</a>".format(link[1], link[0]))
            return refactor_banner.format("<br/>".join(alllinks))
        else:
            return ""


    def get_mastg_v1_coverage(self, meta):
        ...
        if mappings:
            ...
            mastg_v1_tests_map = self.mastg_v1_mapping.get(masvs_v1_id, [])

            mastg_v1_tests_map_list = [f"{test.split(']')[0].split('[')[1]}" for test in mastg_v1_tests_map]
            mappings['mastg-v1'] = mastg_v1_tests_map_list

            mastg_v1_tests = "\n".join([f"    - [{test} - {self.mastg_v1_tests_metadata[test]['title']} ({self.mastg_v1_tests_metadata[test]['platform']})]({self.mastg_v1_tests_metadata[test]['link']})" for test in mastg_v1_tests_map_list])
            if mastg_v1_tests == "":
                mastg_v1_tests = "    No MASTG v1 tests are related to this weakness."
        return mastg_v1_tests

    def get_maswe_draft_banner(self, meta):
        ...
        mastg_v1_tests = self.get_mastg_v1_coverage(meta)
    ...
    def get_tests_draft_banner(self, meta):
        id = meta.get('id')
    ...
    def get_v1_deprecated_tests_banner(self, meta):
        id = meta.get('id')
    ...
    def get_android_demo_buttons(self, page):
        id = page.meta.get('id')
    ...
    def get_ios_demo_buttons(self, page):
        id = page.meta.get('id')
    ...
    def get_demos_draft_banner(self, meta):
        id = meta.get('id')
        note = meta.get('note', None)
