...
The new structure divides tests into individual pages (Markdown files with metadata), each with its own ID (`MASTG-TEST-****`) and links to relevant techniques (`MASTG-TECH-****`) and tools (`MASTG-TOOL-****`). This encapsulation ensures that **each test is easily referenced and promotes reusability** across all MAS components. For example, you can now see which tools are used in a particular test and which techniques are applied. This is currently partially implemented, but when we're done, you'll be able to do many interesting things, like see all tests that use a particular tool or technique.
...
3. **MASTG Tests**: Each weakness is implemented by multiple tests that guide the tester in identifying and mitigating the issues using various tools and techniques on each mobile platform.
   > For example, testing for "insecure random API usage on Android" ([MASTG-TEST-0204](hhttps://mas.owasp.org/MASTG/tests-beta/android/MASVS-CRYPTO/MASTG-TEST-0204/)).
