...
The following example is a snippet from an exemplary Kotlin app decompiled with jadx (#MASTG-TECH-0017.md). From the [static analysis](#check-for-deep-link-usage) we know that it supports the deep link `deeplinkdemo://load.html/` as part of `com.mstg.deeplinkdemo.WebViewActivity`.
